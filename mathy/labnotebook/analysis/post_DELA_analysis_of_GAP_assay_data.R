...
library(pracma)
outfiles <- "final_data"
dir.create(file.path(outfiles), showWarnings = FALSE)

#### analyse and parse parameters from DELA
parameters <- read_tsv('./fit_GAP_data.txt') %>% 
  select(data, kcat, Km, "conc" = `[S]`, basS) %>% 
  separate(col = data, into = c("date", "mutant", "well", "input_conc", "GAP_conc", "sensor_conc"), sep = "-", convert = T, remove = T) %>% 
...
GAP_parameters <- parameters %>% 
  ...
  select(-date, -well, -input_conc, -aprox_loading_eff, -basS)
write_tsv(GAP_parameters, file = file.path(outfiles, "GAP_kinetics_MichaelisMenten_parameters.txt"))
# avg_parameters %>% 
...
avg_parameters %>% 
  ...
  write_delim(file.path(outfiles, 'GAP_table_raw.txt'), delim='\t')
