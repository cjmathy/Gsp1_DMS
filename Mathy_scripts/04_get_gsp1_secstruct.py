...
for pdb in os.listdir(pdb_dir):
    ...
    pose = pyrosetta.pose_from_pdb(os.path.join(pdb_dir, pdb))
    ...
    ss_df['aa_position'] = aa_positions
...
ss_df.to_csv('../data/ss_df_long.csv', index=False)

# pivot the dataframe to wide format so that each row is a position
# and each column is a different PDB
ss_df_wide = ss_df.pivot(index='aa_position', columns='pdb_id', values='ss')
temp_df = pd.DataFrame(data={'aa_position': list(range(1,220)),
                             'gsp1_aa': list(gsp1_record.seq)})
ss_df_wide = temp_df.merge(ss_df_wide, on='aa_position', how='left')
...
ss_df_wide.to_csv('../data/ss_df_wide.csv', index=False)
